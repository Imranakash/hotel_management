...
class HotelHousekeepingTask(models.Model):
    ...
    assigned_to = fields.Many2one('res.users', string='Assigned Staff', tracking=True,domain=lambda self: [('groups_id', 'in', [self.env.ref('hotel_housekeeping.group_housekeeping_staff').id])])
    ...
    checklist_id = fields.Many2one('hotel.housekeeping.checklist', string='Checklist Template', tracking=True)
    ...
    @api.depends('task_no', 'room_id')
    def _compute_display_name(self):
        for record in self:
            res_name = record.task_no or ''
            if record.room_id:
                res_name = f"{res_name} - {record.room_id.display_name}"
