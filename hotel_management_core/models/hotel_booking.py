...
class HotelBooking(models.Model):
    ...
    cancellation_fee = fields.Float(
        string='Cancellation Charge',
        readonly=True,
        copy=False,
        help="Amount charged as cancellation fee"
    )
    ...
    guest_id_number = fields.Char(string='ID / Document Number',  tracking=True)

    is_locked_for_edit = fields.Boolean(compute='_compute_is_locked_for_edit')


    is_late_checkin = fields.Boolean(
        string="Late Check-In Alert",
        compute="_compute_is_late_checkin",
        store=True
    )

    @api.depends('state', 'checkin_date')
    def _compute_is_late_checkin(self):
        today = fields.Date.context_today(self)
        for rec in self:

            if rec.state == 'confirmed' and rec.checkin_date and rec.checkin_date < today:
                rec.is_late_checkin = True
            else:
                rec.is_late_checkin = False

    is_overdue_checkout = fields.Boolean(
        string="Overdue Check-Out",
    ...
    def _compute_is_overdue_checkout(self):
        today = fields.Date.context_today(self)
        for record in self:
            if record.state == 'checked_in' and record.checkout_date and record.checkout_date < today:
                record.is_overdue_checkout = True
            else:
    ...
    def action_cancel_booking(self):
        ...
        checkin_date = fields.Date.to_date(self.checkin_date) if self.checkin_date else today
        days_left = (checkin_date - today).days

        policy = self.env['hotel.cancellation.policy'].search([
            ('days_before', '<=', days_left)
        ], order='days_before desc', limit=1)

        refund_percent = policy.refund_percentage if policy else 0.0

        advance = getattr(self, 'advance_deposit_received', 0.0) or getattr(self, 'advance_amount', 0.0) or 0.0
        refund_amt = (advance * refund_percent) / 100.0
        fee_amt = advance - refund_amt

        wizard = self.env['hotel.booking.cancel.wizard'].create({
            'booking_id': self.id,
        ...
        return {
            'name': 'Cancel Booking & Refund Details',
            'type': 'ir.actions.act_window',
            'res_model': 'hotel.booking.cancel.wizard',
            'res_id': wizard.id,
            'view_mode': 'form',
            'target': 'new',
        }
    ...
    def _compute_is_locked_for_edit(self):
        for record in self:
            record.is_locked_for_edit = record.state in ['confirmed', 'checked_in', 'checked_out']
    ...
    def action_confirm(self):
        for record in self:
            # MINIMUM ADVANCE DEPOSIT CHECK
            property_obj = record.property_id
            if property_obj and property_obj.min_deposit_amount > 0:
                if record.deposit_amount < property_obj.min_deposit_amount:
                    raise UserError(_(
                        "🛑 Advance Deposit Required!\n\n"
                        "Minimum advance deposit for '%s' is %s BDT.\n"
                        "Current Deposit Amount: %s BDT.\n\n"
                        "Please collect at least %s BDT advance deposit to confirm this booking."
                    ) % (property_obj.name, property_obj.min_deposit_amount, record.deposit_amount,
                         property_obj.min_deposit_amount))
            ...
            if record.discount_amount and not record.is_approved:
                ...
                for line in record.room_line_ids:
                    if line.room_type_id and record.property_id:
                        ...
                        if rate_setup:
                            qty = getattr(line, 'room_qty', 1) or 1
                            allowed_max_discount += (rate_setup.max_discount * qty)
    ...
    def action_check_in(self):
        ...
        if self.state != 'confirmed':
            return False
        today = fields.Date.context_today(self)
        if self.checkin_date and today < self.checkin_date:
            raise UserError(
                "🛑 Early Check-In Not Allowed!\n\n"
                f"Scheduled Arrival Date: {self.checkin_date}\n"
                f"Today's Date: {today}\n\n"
                "You cannot check-in guest before the arrival date."
            )
        ...
        return {
            'name': 'Select Available Rooms for Check-In',
            'type': 'ir.actions.act_window',
    ...
    def action_cancel(self):
        ...
        refund_pct = policy.refund_percentage if policy else 0.0

        advance = self.deposit_amount or 0.0

        refund_amt = (advance * refund_pct) / 100.0
        fee_amt = advance - refund_amt
        ...
        return {
            'name': _('Cancel Booking & Refund Details'),
            'type': 'ir.actions.act_window',
            'res_model': 'hotel.booking.cancel.wizard',
            'res_id': wizard.id,
            'view_mode': 'form',
    ...
    def write(self, vals):
        import logging
        logging.getLogger(__name__).info("###WRITE### state=%s vals=%s", self.mapped('state'), vals)
        if not vals:
        ...
        for record in self:
            ...
            if record.state in ['confirmed', 'checked_in', 'checked_out']:
                if 'state' in vals and vals['state'] in ['draft', 'cancelled']:
                    continue

                bypass_fields = ['state', 'payment_state', 'cancellation_date', 'forfeiture_amount',
                                 'refund_amount', 'commission_status', 'message_follower_ids', 'activity_ids',
                                 'message_ids', 'guest_id_type', 'guest_id_number', 'room_line_ids']
                real_fields_changed = [
                    field for field in vals.keys()
                    if field not in bypass_fields and vals[field] != record[field]
                ]

                if real_fields_changed:
                    logging.getLogger(__name__).info("###BLOCKED### fields=%s", real_fields_changed)
                    raise UserError(
...
class HotelBookingRoomLine(models.Model):
    ...
    booking_state = fields.Selection(related='booking_id.state', string="Booking State", store=True)
    ...
    room_availability_display = fields.Char(
        string='Availability',
        compute='_compute_room_availability_display',
    )

    @api.depends('room_type_id', 'booking_id.property_id',
                 'booking_id.checkin_date', 'booking_id.checkout_date')
    def _compute_room_availability_display(self):
        for line in self:
            if not line.room_type_id or not line.booking_id.property_id:
                line.room_availability_display = ''
                continue
            ...
            if line.booking_id.checkin_date and line.booking_id.checkout_date:
                overlapping_lines = self.env['hotel.booking.room.line'].search([
                    ('room_type_id', '=', line.room_type_id.id),
                    ('booking_id.property_id', '=', line.booking_id.property_id.id),
                    ('booking_id.state', 'in', ['confirmed', 'checked_in']),
                ...
                booked_room_ids = overlapping_lines.mapped('room_id.id')

            available_count = total_rooms - len(set(booked_room_ids))
            line.room_availability_display = _("%s Available / %s Total") % (available_count, total_rooms)
    ...
    # ১. মডেলে নতুন ফিল্ড যোগ করুন
    room_qty = fields.Integer(string='Room Quantity', default=1, required=True)

    # ২. আগের Validation মেথডটি আপডেট করুন (requested_lines এর জায়গায় মোট কয়ান্টিটি চেক হবে)
    @api.constrains('room_type_id', 'booking_id', 'room_qty')
    def _check_available_room_limit(self):
        for line in self:
            if not line.booking_id or not line.room_type_id or not line.property_id:
                continue
            ...
            if line.booking_id.checkin_date and line.booking_id.checkout_date:
                overlapping_lines = self.env['hotel.booking.room.line'].search([
                    ('room_type_id', '=', line.room_type_id.id),
                    ('booking_id.property_id', '=', line.property_id.id),
                    ('booking_id.state', 'in', ['confirmed', 'checked_in']),
                    ('booking_id', '!=', line.booking_id.id),
                    ('booking_id.checkin_date', '<', line.booking_id.checkout_date),
                    ('booking_id.checkout_date', '>', line.booking_id.checkin_date),
            ...
            # একই টাইপের সব লাইনের room_qty যোগ করা হচ্ছে
            total_requested_qty = sum(line.booking_id.room_line_ids.filtered(
                lambda l: l.room_type_id == line.room_type_id
            ).mapped('room_qty'))

            if total_requested_qty > available_count:
                raise ValidationError(_(
                    "🛑 Overbooking Blocked!\n\n"
                    "Category '%s' has only %s room(s) available.\n"
                    "You requested %s rooms."
                ) % (line.room_type_id.name, max(0, available_count), total_requested_qty))
