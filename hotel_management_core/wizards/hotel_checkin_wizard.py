...
class HotelCheckinWizard(models.TransientModel):
    ...
    def action_assign_rooms_and_confirm_checkin(self):
        ...
        for wizard_line in self.line_ids:
            unassigned_line = booking.room_line_ids.filtered(
                lambda l: l.room_type_id == wizard_line.room_type_id and not l.room_id
            )

            if unassigned_line:
                target_line = unassigned_line[0]
                qty = getattr(target_line, 'room_qty', 1) or 1

                if qty > 1:
                    target_line.write({
                        'room_id': wizard_line.room_id.id,
                        'room_qty': 1
                    })
                    booking.room_line_ids.create({
                        'booking_id': booking.id,
                        'room_type_id': target_line.room_type_id.id,
                        'room_qty': qty - 1,
                        'price_unit': target_line.price_unit,
                        'extra_bed_count': target_line.extra_bed_count,
                    })
                else:
                    target_line.write({'room_id': wizard_line.room_id.id})
