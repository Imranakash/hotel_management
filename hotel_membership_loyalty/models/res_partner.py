...
class ResPartner(models.Model):
    ...
    membership_card_no = fields.Char(string='Membership Card No', copy=False, readonly=True,tracking=True)
    membership_tier_id = fields.Many2one('hotel.membership.tier', string='VIP Tier', compute='_compute_membership_tier', store=True,tracking=True)
    loyalty_points_balance = fields.Integer(string='Loyalty Points Balance', compute='_compute_loyalty_points', store=True,tracking=True)
    total_points_earned = fields.Integer(string='Total Points Earned', compute='_compute_loyalty_points', store=True,tracking=True)
