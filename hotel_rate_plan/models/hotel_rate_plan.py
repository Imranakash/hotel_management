...
class HotelRatePlan(models.Model):
    ...
    tax_included = fields.Boolean(string="Tax Included", default=False, tracking=True)
    service_charge_included = fields.Boolean(string="Service Charge Included", default=False, tracking=True)
...
class HotelProperty(models.Model):
    ...
    ], string="Early Check-In Policy", default='free',tracking=True)
    early_checkin_fee = fields.Float(string="Early Check-In Fee",tracking=True)

    late_checkout_charge_type = fields.Selection([
        ('fixed', 'Fixed Amount'),
        ('percentage', 'Percentage of Room Rate'),
        ('free', 'Complimentary / Free')
    ], string="Late Check-Out Policy", default='free',tracking=True)
    late_checkout_fee = fields.Float(string="Late Check-Out Fee",tracking=True)